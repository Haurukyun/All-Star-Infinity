...
export const SkyrimPlayScreen: React.FC<{ logic: any }> = ({ logic }) => {
    ...
    return (
        <AnimatePresence mode="wait">
            {!intensity && !prompt ? (
                <motion.div key="play" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="flex-1 grid grid-cols-1 md:grid-cols-2 gap-8 items-start">
                    <div className="space-y-6">
                        <h2 className="skyrim-font-title text-xl text-white/70 mb-4 border-b border-white/20 pb-2">ACTIVE QUESTS</h2>
                        <div className="flex flex-col">
                            <button onClick={() => setActiveDeckId('default')} className={`skyrim-btn ${activeDeckId === 'default' ? 'active' : ''}`}>Standard Adventure</button>
                            {customDecks.map((deck: any) => (
                                <button key={deck.id} onClick={() => setActiveDeckId(deck.id)} className={`skyrim-btn ${activeDeckId === deck.id ? 'active' : ''}`}>{deck.name}</button>
                            ))}
                        </div>
                    </div>
                    <div className="skyrim-panel p-6 min-h-[300px] flex flex-col">
                        ...
                        <div className="flex-1 flex flex-col justify-center gap-4">
                            {STAGES.map((stage) => (
                                <button key={stage.id} onClick={() => setIntensity(stage.id)} className="group flex items-center justify-between p-2 hover:bg-white/5 transition-colors">
                                    <div className="flex flex-col text-left"><span className="skyrim-font-title text-lg group-hover:text-yellow-100 transition-colors">{stage.title}</span><span className="text-xs uppercase tracking-widest opacity-60">{stage.desc}</span></div>
                                    <div className="w-2 h-2 bg-white/50 rotate-45 group-hover:bg-white transition-colors"></div>
                                </button>
                            ))}
                        </div>
                        <div className="mt-6 pt-4 border-t border-white/10 text-center"><p className="text-sm italic opacity-60">"Choose your path, Dragonborn."</p></div>
                    </div>
                </motion.div>
            ) : !prompt ? (
                <div className="flex-1 flex flex-col items-center justify-center relative min-h-[50vh]">
                    <div className="absolute inset-0 flex items-center justify-center opacity-10 pointer-events-none">
                        <svg viewBox="0 0 100 100" className="w-96 h-96 animate-spin-slow"><path d="M50 10 L90 50 L50 90 L10 50 Z" fill="none" stroke="white" strokeWidth="1" /><circle cx="50" cy="50" r="30" fill="none" stroke="white" strokeWidth="1" /></svg>
                    </div>
                    ...
                    <div className="flex gap-12">
                        <button onClick={() => handleDraw('Truth')} className="group flex flex-col items-center gap-2">
                            <div className="w-16 h-16 border border-white/30 rotate-45 flex items-center justify-center group-hover:border-white group-hover:bg-white/10 transition-all"><span className="text-2xl -rotate-45">👁️</span></div>
                    ...
                    <button onClick={() => setIntensity(null)} className="mt-16 text-xs uppercase tracking-[3px] opacity-50 hover:opacity-100 transition-opacity">[ TAB ] Cancel</button>
                </div>
            ) : (
            ...
            {!editingDeck ? (
                <div className="grid grid-cols-1 md:grid-cols-3 gap-8 h-full">
                    <div className="md:col-span-1 border-r border-white/10 pr-4">
                        ...
                        <div className="flex flex-col">
                            <button onClick={() => setEditingDeck({ id: generateId(), name: '', description: '', prompts: [], isCustom: true })} className="skyrim-btn text-yellow-100">+ CRAFT NEW ITEM</button>
                            <div className="h-px bg-white/20 my-2 mx-4"></div>
                            {customDecks.map((deck: any) => (
                                <div key={deck.id} className="flex justify-between items-center pr-4 group"><button onClick={() => setEditingDeck(deck)} className="skyrim-btn flex-1 truncate">{deck.name || 'Iron Dagger'}</button><button onClick={() => deleteDeck(deck.id)} className="opacity-0 group-hover:opacity-50 hover:!opacity-100 text-xs uppercase tracking-widest">DROP</button></div>
                            ))}
                        </div>
                    </div>
                    <div className="hidden md:flex md:col-span-2 items-center justify-center opacity-30"><div className="text-center"><div className="text-6xl mb-4">🎒</div><p className="skyrim-font-title tracking-widest">SELECT AN ITEM TO INSPECT</p></div></div>
                </div>
            ) : (
                <div className="max-w-xl mx-auto space-y-8 pb-12">
                    <div className="border-b border-white/20 pb-4 flex justify-between items-end"><h2 className="skyrim-font-title text-3xl">ENCHANTING</h2><span className="text-xs tracking-widest opacity-50">ARCANE ENCHANTER</span></div>
                    <div className="space-y-6">
                        <div><label className="text-xs uppercase tracking-widest opacity-50 block mb-2">Item Name</label><input className="skyrim-input text-xl" value={editingDeck.name} onChange={e => setEditingDeck({ ...editingDeck, name: e.target.value })} placeholder="Name your creation..." /></div>
                        <div>
                            <div className="flex justify-between items-center mb-4"><label className="text-xs uppercase tracking-widest opacity-50">Effects ({editingDeck.prompts.length})</label><button onClick={addNewPromptToEditingDeck} className="text-xs uppercase tracking-widest hover:text-yellow-100 border border-white/20 px-2 py-1">+ Add Effect</button></div>
                            <div className="max-h-[30vh] overflow-y-auto custom-scrollbar space-y-2">
                                {editingDeck.prompts.map((p: any) => (
                                    <div key={p.id} className="bg-white/5 p-3 flex gap-4 items-start border border-white/10">
                                        <select className="bg-black border border-white/20 text-xs uppercase p-1 outline-none" value={p.type} onChange={e => updatePromptInEditingDeck(p.id, 'type', e.target.value)}><option>Truth</option><option>Dare</option></select>
                    ...
                    <div className="flex justify-end gap-6 pt-4 border-t border-white/10"><button onClick={() => setEditingDeck(null)} className="text-sm uppercase tracking-widest opacity-70 hover:opacity-100">Cancel</button><button onClick={() => saveDeck(editingDeck)} className="text-sm uppercase tracking-widest font-bold hover:text-yellow-100">Craft Item</button></div>
                </div>
            )}
        </AnimatePresence>
    );
