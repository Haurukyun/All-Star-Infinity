...
export const OmoriLayout: React.FC<{ children: React.ReactNode; activeTab: string; setActiveTab: (tab: string) => void; logic: any }> = ({ children, activeTab, setActiveTab, logic }) => {
    const { intensity } = logic;
    const tabs = [
        { id: 'play', label: 'DREAM' },
                ...
                <motion.div key="play" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="space-y-8">
                    <div className="omori-panel p-6">
                        ...
                        <div className="grid grid-cols-1 gap-3">
                            <button onClick={() => setActiveDeckId('default')} className={`omori-button ${activeDeckId === 'default' ? 'active' : ''}`}>STILL HERE</button>
                            {customDecks.map((deck: any) => (
                                <button key={deck.id} onClick={() => setActiveDeckId(deck.id)} className={`omori-button ${activeDeckId === deck.id ? 'active' : ''}`}>{deck.name}</button>
                            ))}
                        </div>
                    </div>
                    <div className="omori-panel p-6">
                        ...
                        <div className="grid grid-cols-1 gap-4">
                            {STAGES.map((stage) => (
                                <button key={stage.id} onClick={() => setIntensity(stage.id)} className="omori-button flex flex-col items-center py-4" style={{ backgroundColor: stage.color, color: stage.text }}>
...
export const OmoriHistoryScreen: React.FC<{ logic: any }> = ({ logic }) => {
    const { history } = logic;
    return (
            ...
            <div className="space-y-3">
                {history.map((item: any, i: number) => (
                    <div key={i} className="omori-panel p-4">
                        <div className="flex justify-between text-xs mb-2 opacity-60"><span>{item.type.toUpperCase()}</span><span>LOG {history.length - i}</span></div>
...
export const OmoriSettingsScreen: React.FC<{ logic: any }> = ({ logic }) => {
    const { setView } = logic;
    return (
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-6">
            <h2 className="text-3xl border-b-2 border-black pb-1">INNER HEART</h2>
            <div className="omori-panel p-6 space-y-6">
                <div className="flex justify-between items-center"><span>HAPPINESS</span><span className="font-bold">LOW</span></div>
                <div className="flex justify-between items-center"><span>SADNESS</span><span className="font-bold">HIGH</span></div>
                <div className="flex justify-between items-center"><span>ANXIETY</span><span className="font-bold animate-pulse">PEAK</span></div>
                <div className="pt-4 border-t border-black/10"><p className="text-xs opacity-40 text-center">Everything is going to be okay.</p></div>
                <button onClick={() => logic.setView('menu')} className="w-full omori-button text-xs mt-4">WAKE UP</button>
            </div>
        </motion.div>
    );
};

export const OmoriMenu: React.FC<{ logic: any }> = ({ logic }) => {
    ...
    return (
        <div className="h-full w-full flex flex-col items-center justify-center p-6 bg-white font-['Gloria_Hallelujah'] text-black relative overflow-hidden select-none">
            <style>{`
                @import url('https://fonts.googleapis.com/css2?family=Gloria+Hallelujah&display=swap');
                .sketchy-border { border: 2px solid #000; transition: all 0.2s; cursor: pointer; position: relative; }
                .sketchy-border:hover { transform: translate(-2px, -2px); box-shadow: 4px 4px 0px #000; }
            ...
            <div className="z-10 flex flex-col items-center max-w-sm w-full">
                <div className="relative mb-12">
                    <motion.div className="absolute -inset-8 border-2 border-dashed border-black/5 rounded-full" animate={{ rotate: 360 }} transition={{ duration: 20, repeat: Infinity, ease: "linear" }} />
                    <motion.h1 className="text-7xl sm:text-9xl font-bold tracking-tighter text-black relative" animate={{ y: [0, -2, 0] }} transition={{ duration: 4, repeat: Infinity, ease: "easeInOut" }}>OMORI</motion.h1>
                ...
                <div className="flex flex-col gap-6 w-full px-8">
                    <button onClick={() => setView('game')} className="group relative py-4 transition-all">
                    ...
                    <button onClick={() => setActiveSection(activeSection === 'themes' ? null : 'themes')} className="group relative py-4 transition-all">
                ...
                <AnimatePresence>
                    {activeSection === 'themes' && (
                        <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.95 }} className="mt-8 w-full bg-white border-2 border-black p-6 shadow-[8px_8px_0_#000] max-h-48 overflow-y-auto no-scrollbar" style={{ borderRadius: '15px 5px 20px 8px' }}>
                            <div className="grid grid-cols-1 gap-4">
                                {themes.map(t => (
                                    <button key={t} onClick={() => setTheme(t)} className={`text-left text-lg hover:underline decoration-wavy transition-all ${theme === t ? 'font-bold' : 'opacity-40'}`}>{t.replace('_', ' ')}</button>
                                ))}
                            </div>
                        </motion.div>
                    )}
            ...
            <div className="absolute bottom-10 left-10 opacity-10 text-[10px] max-w-[150px] leading-tight">You have been living here for as long as you can remember.</div>
        </div>
    );
};

export const OmoriTheme: ThemeDefinition = {
    id: Theme.OMORI,
    name: 'Omori',
    cssVars: {
        '--theme-accent': '#000',
    },
    MenuComponent: OmoriMenu,
    LayoutComponent: OmoriLayout,
    PlayScreen: OmoriPlayScreen,
    DecksScreen: OmoriDecksScreen,
    HistoryScreen: OmoriHistoryScreen,
    SettingsScreen: OmoriSettingsScreen,
    ThemesScreen: OmoriThemesScreen,
    tabLabels: {
        play: 'DREAM',
        decks: 'MEMORIES',
        history: 'TRUTH',
