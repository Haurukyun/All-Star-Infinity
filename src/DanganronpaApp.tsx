...
const DanganronpaApp: React.FC<{ logic: ReturnType<typeof useGameLogic> }> = ({ logic }) => {
  ...
  return (
    <div className="dr-theme h-[100dvh] w-screen flex flex-col bg-[#1a1a1a] text-white overflow-hidden font-['Orbitron']">
      <style>{`
        .dr-theme {
          background-image: 
            ...
            {activeTab === 'themes' && (
              <motion.div key="themes" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="space-y-6">
                ...
                <div className="grid grid-cols-1 gap-4">
                  ...
                  <button onClick={() => setTheme(Theme.KIRBY)} className="dr-button py-6 text-xl">DREAM LAND</button>
                </div>
              </motion.div>
            )}
      ...
      {/* Navigation */}
      <nav className="fixed bottom-0 left-0 w-full bg-black/90 border-t-2 border-[#FF00FF] h-24 z-50">
