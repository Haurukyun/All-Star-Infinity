...
const AnimalCrossingApp: React.FC<{ logic: ReturnType<typeof useGameLogic> }> = ({ logic }) => {
  ...
  const tabs = [
    { id: 'play', label: 'ISLAND', icon: '🏝️', color: '#A5D6A7' },
    { id: 'decks', label: 'DIY', icon: '🔨', color: '#FFCC80' },
    { id: 'history', label: 'MILES', icon: '🎫', color: '#90CAF9' },
    { id: 'themes', label: 'TRAVEL', icon: '✈️', color: '#CE93D8' },
    { id: 'settings', label: 'PASSPORT', icon: '🛂', color: '#B0BEC5' },
  ];
    ...
    <div className="ac-theme h-[100dvh] w-screen flex flex-col bg-[#F0F4C3] text-[#5D4037] overflow-hidden font-['Varela_Round'] relative">
      <style>{`
        ...
        .ac-theme {
          ...
          background-image: 
            radial-gradient(#C5E1A5 15%, transparent 16%),
            radial-gradient(#C5E1A5 15%, transparent 16%);
                    ...
                    <div className="ac-panel p-6">
                      ...
                      <div className="grid grid-cols-1 gap-3">
                        <button 
                          ...
                          className={`ac-card flex items-center gap-4 transition-all ${activeDeckId === 'default' ? 'ring-4 ring-[#81D4FA]' : ''}`}
                        >
                          <div className="w-12 h-12 bg-[#B2DFDB] rounded-full flex items-center justify-center text-2xl">
                            ⛺
                          </div>
                          <div className="text-left">
                            <div className="font-bold text-lg">Island Life</div>
                            <div className="text-xs text-gray-500">Standard Experience</div>
                          </div>
                        </button>
                        {customDecks.map(deck => (
                          <button 
                            ...
                            className={`ac-card flex items-center gap-4 transition-all ${activeDeckId === deck.id ? 'ring-4 ring-[#81D4FA]' : ''}`}
                          >
                            <div className="w-12 h-12 bg-[#FFCC80] rounded-full flex items-center justify-center text-2xl">
                              📦
                            </div>
                            <div className="text-left">
                              <div className="font-bold text-lg">{deck.name}</div>
                              <div className="text-xs text-gray-500">{deck.prompts.length} Tasks</div>
                            </div>
                          </button>
                        ))}
                      </div>
                    ...
                    <div className="grid grid-cols-1 gap-4">
                      {STAGES.map((stage) => (
                        <button 
                          ...
                          className="ac-card flex items-center justify-between p-4 hover:scale-105 transition-transform"
                        ...
                        >
                          <div className="flex items-center gap-4">
                            ...
                            <div className="text-left">
                              <div className="font-bold text-lg text-white drop-shadow-sm">{stage.title}</div>
                              <div className="text-xs text-white/80 font-bold">{stage.desc}</div>
                            </div>
                          </div>
                  ...
                  <div className="flex flex-col items-center justify-center h-[60vh] relative">
                    <div className="ac-bubble w-full text-center mb-8">
                      ...
                      <p className="text-xl text-[#5D4037]">
                        "Ready for a new task, hm? Choose wisely, yes, yes!"
                      </p>
                    </div>
                    ...
                    <div className="flex gap-4 w-full px-4">
                      <button onClick={() => handleDraw('Truth')} className="flex-1 ac-btn bg-[#4DD0E1] text-white border-[#B2EBF2]">
                        TRUTH
                      </button>
                      <button onClick={() => handleDraw('Dare')} className="flex-1 ac-btn bg-[#FF7043] text-white border-[#FFCCBC]">
                        DARE
                      </button>
                    </div>
                    
                    <button onClick={() => setIntensity(null)} className="mt-8 text-sm font-bold text-white bg-[#8D6E63] px-4 py-2 rounded-full shadow-sm hover:bg-[#795548]">
                      Nevermind
                    </button>
                  </div>
                ) : (
                  <motion.div initial={{ y: 50, opacity: 0 }} animate={{ y: 0, opacity: 1 }} className="relative mt-4">
                    <div className="ac-panel p-6 border-[#FFF9C4]">
                      ...
                      <div className="text-center space-y-6 pt-4">
                        <div className="w-16 h-16 mx-auto bg-[#E1F5FE] rounded-full flex items-center justify-center text-3xl border-4 border-white shadow-sm">
                          {prompt.type === 'Truth' ? '🤔' : '⚡'}
                        </div>
                    ...
                    <div className="flex gap-3 mt-6">
                      <button onClick={() => setPrompt(null)} className="flex-1 ac-btn bg-[#AED581] text-white border-[#DCEDC8]">
                        Done!
                      </button>
                      <button onClick={() => handleDraw(prompt.type)} className="flex-1 ac-btn bg-[#4DB6AC] text-white border-[#B2DFDB]">
                        Again!
                      </button>
                    </div>
                  </motion.div>
                ...
                {!editingDeck ? (
                    ...
                    <div className="ac-panel p-4 bg-[#FFF3E0]">
                      ...
                      <div className="grid grid-cols-2 gap-3">
                        {customDecks.length === 0 ? (
                          <div className="col-span-2 text-center py-8 text-gray-400 font-bold">No recipes yet!</div>
                        ) : (
                          customDecks.map(deck => (
                            <div key={deck.id} className="ac-card flex flex-col items-center text-center gap-2 hover:scale-105 transition-transform cursor-pointer" onClick={() => setEditingDeck(deck)}>
                              <div className="w-12 h-12 bg-[#FFE0B2] rounded-full flex items-center justify-center text-2xl">
                                📜
                              </div>
                              <div>
                                <div className="font-bold text-sm truncate w-full">{deck.name || 'Untitled'}</div>
                  ...
                  <div className="ac-panel p-6 space-y-4 bg-[#FFF3E0]">
                    <div className="text-center mb-4">
                      <h2 className="text-xl font-bold text-[#E65100]">Crafting Recipe</h2>
                    </div>
                    ...
                    <div className="space-y-2">
                      <div className="flex justify-between items-center px-2">
                        <span className="font-bold text-[#E65100]">Materials ({editingDeck.prompts.length})</span>
                        <button onClick={addNewPromptToEditingDeck} className="text-[#E65100] text-sm font-bold bg-[#FFE0B2] px-2 py-1 rounded-full">+ Add</button>
                      </div>
                      <div className="max-h-[40vh] overflow-y-auto space-y-2 pr-1 custom-scrollbar">
                        {editingDeck.prompts.map(p => (
                          <div key={p.id} className="bg-white rounded-xl p-2 flex gap-2 items-center shadow-sm">
                            <select className="bg-[#FFE0B2] text-[#E65100] text-xs font-bold rounded-lg p-1 outline-none" value={p.type} onChange={e => updatePromptInEditingDeck(p.id, 'type', e.target.value)}>
                    ...
                    <div className="flex gap-2 pt-2">
                      <button onClick={() => setEditingDeck(null)} className="flex-1 bg-[#BCAAA4] text-white font-bold py-3 rounded-xl shadow-sm">Cancel</button>
                      <button onClick={() => saveDeck(editingDeck)} className="flex-1 bg-[#FFB74D] text-white font-bold py-3 rounded-xl shadow-sm">Save</button>
                    </div>
                  </div>
                )}
              </motion.div>
              ...
              <motion.div key="history" initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-4">
                <div className="ac-panel p-4 bg-[#E1F5FE]">
                  <div className="flex items-center gap-2 mb-4 border-b-2 border-white pb-2">
                    ...
                    <h2 className="text-xl font-bold text-[#0288D1]">Nook Miles</h2>
                  </div>
                  ...
                  <div className="grid grid-cols-3 gap-3">
                    {history.map((item, i) => (
                      <div key={i} className="bg-white rounded-xl p-2 flex flex-col items-center gap-2 shadow-sm text-center aspect-square justify-center">
                        <div className={`stamp ${item.type ? 'filled' : ''}`}>
                          {item.type === 'Truth' ? '🤔' : '⚡'}
                        </div>
                        <div className="text-[10px] font-bold text-[#0288D1] leading-tight line-clamp-2">
                          {item.text}
                        </div>
                      </div>
                    ))}
                    {Array.from({ length: Math.max(0, 9 - history.length) }).map((_, i) => (
                      <div key={`empty-${i}`} className="bg-white/50 rounded-xl p-2 flex items-center justify-center aspect-square">
              ...
              <motion.div key="themes" initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-4">
                <div className="ac-panel p-4 bg-[#E1BEE7]">
                  <div className="flex items-center gap-2 mb-4">
                    ...
                    <h2 className="text-xl font-bold text-[#8E24AA]">Dodo Airlines</h2>
                  </div>
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    {[
                      ...
                      <button 
                        ...
                        className={`ac-card flex flex-col items-center justify-center gap-2 py-4 hover:scale-105 transition-transform ${theme === t.id ? 'ring-4 ring-[#AB47BC]' : ''}`}
                      >
              ...
              <motion.div key="settings" initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-4">
                <div className="ac-panel p-6 bg-[#CFD8DC]">
                  <div className="flex items-center gap-4 mb-6">
                    <div className="w-16 h-16 bg-white rounded-lg border-4 border-white shadow-sm flex items-center justify-center text-3xl">
                      🙂
                    </div>
                    <div>
                      <h2 className="text-2xl font-bold text-[#455A64]">PASSPORT</h2>
                      <p className="text-[#78909C] font-bold">Island Resident</p>
                    </div>
                  </div>
                  
                  <div className="space-y-4">
                    <div className="bg-white rounded-xl p-3 shadow-sm">
                      <p className="text-xs text-[#90A4AE] font-bold uppercase">Island Name</p>
                      <p className="text-lg font-bold text-[#455A64]">Paradise</p>
                    </div>
                    <div className="bg-white rounded-xl p-3 shadow-sm">
                      <p className="text-xs text-[#90A4AE] font-bold uppercase">Title</p>
                      <p className="text-lg font-bold text-[#455A64]">Full-fledged Truth Seeker</p>
                    </div>
                    <div className="bg-white rounded-xl p-3 shadow-sm">
                      <p className="text-xs text-[#90A4AE] font-bold uppercase">Birthday</p>
                      <p className="text-lg font-bold text-[#455A64]">March 20th</p>
                    </div>
                  </div>
                  
                  <div className="mt-6 flex justify-center">
                    <div className="w-12 h-12 rounded-full border-2 border-[#B0BEC5] flex items-center justify-center opacity-50">
  ...
  return (
    <div className="h-full w-full flex flex-col items-center justify-center p-6 bg-[#F0F4C3] font-sans text-[#5D4037] relative overflow-hidden">
      ...
      <div className="z-10 flex flex-col items-center gap-12">
        <motion.h1 
          className="text-6xl font-black tracking-tight text-[#795548] drop-shadow-sm"
        ...
        <AnimatePresence>
          {activeSection === 'themes' && (
            <motion.div initial={{ opacity: 0, scale: 0.8 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.8 }} className="grid grid-cols-2 gap-3 bg-[#FFF9C4] p-6 rounded-[40px] border-4 border-white shadow-lg max-h-48 overflow-y-auto custom-scrollbar">
              {themes.map(t => (
                <button key={t} onClick={() => setTheme(t)} className="bg-white/50 rounded-full px-4 py-2 text-[10px] font-bold hover:bg-[#8BC34A] hover:text-white transition-all">{t.toUpperCase()}</button>
              ))}
            </motion.div>
          )}
        </AnimatePresence>
